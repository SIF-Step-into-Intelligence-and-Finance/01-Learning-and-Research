...
class Model_Mine(Model):
    def __init__(self, n_mid, embedding_dim, hidden_size, batch_size, num_interest, cand_num, seq_len=256, f_mycand=True, f_encoder=True, sa_dim=64, f_trans=True, trans_k=7, trans_h=16, trans_p=1.0, f_auxloss=True, loss_k=1.0):
        super(Model_Mine, self).__init__(n_mid, embedding_dim, hidden_size, batch_size, seq_len, flag="Model_Mine")
        ...
        self.position_embedding = tf.get_variable(shape=[1, seq_len, embedding_dim], name='position_embedding')  # (1,seq_len,embed_dim)
        ...
        with tf.variable_scope("activ_cand", reuse=tf.AUTO_REUSE) as scope:
            ...
            if f_trans:
                ...
                logits_out = tf.einsum('bi,ij->bj', logits_out, self.trans_matrix)  # (batch,cand_num)
        ...
        if f_auxloss:
            ...
            aux_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=aim_cand_hit, logits=logits_out))
    ...
    def output_info(self, sess, inps):
        user_embs, intere_idx = sess.run((self.user_eb, self.intere_idx), feed_dict={self.mid_his_batch_ph: inps[0], self.mask: inps[1]})
